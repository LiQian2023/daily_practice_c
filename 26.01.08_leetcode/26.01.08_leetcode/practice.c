...
int Get_Min(int* first, int* firstDuration, int n, int* second, int* secondDuration, int m) {
	int start = first[0] + firstDuration[0];
	for (int i = 1; i < n; i++) {
		int tmp = first[i] + firstDuration[i];
		if (start > tmp) {
			start = tmp;
		}
	}
	int res = start + second[0] + secondDuration[0];
	for (int i = 0; i < m; i++) {
		int tmp = start > second[i] ? start : second[i];
		tmp += secondDuration[i];
		res = res < tmp ? res : tmp;
	}
	return res;
}
...
int earliestFinishTime(int* landStartTime, int landStartTimeSize, int* landDuration, int landDurationSize, int* waterStartTime, int waterStartTimeSize, int* waterDuration, int waterDurationSize) {
	int land = Get_Min(landStartTime, landDuration, landStartTimeSize, waterStartTime, waterDuration, waterStartTimeSize);
	int water = Get_Min(waterStartTime, waterDuration, waterStartTimeSize, landStartTime, landDuration, landStartTimeSize);
	return land < water ? land : water;
}