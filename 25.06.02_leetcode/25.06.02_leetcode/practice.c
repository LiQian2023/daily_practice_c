...
void DFS(int** image, int m, int n, int** ans, int i, int j, int key, int color) {
	if (i < 0 || i >= m || j < 0 || j >= n || ans[i][j] == color) {
		return;
	}
	if (image[i][j] == key) {
		ans[i][j] = color;
	...
	//上
	DFS(image, m, n, ans, i - 1, j, key, color);
	// 下
	DFS(image, m, n, ans, i + 1, j, key, color);
	// 左
	DFS(image, m, n, ans, i, j - 1, key, color);
	// 右
	DFS(image, m, n, ans, i, j + 1, key, color);
}
...
int** floodFill(int** image, int imageSize, int* imageColSize, int sr, int sc, int color, int* returnSize, int** returnColumnSizes) {
	...
	*returnColumnSizes = (int*)calloc(imageSize, sizeof(int*));
	...
	DFS(image, imageSize, imageColSize[sr], ans, sr, sc, image[sr][sc], color);
	return ans;
}