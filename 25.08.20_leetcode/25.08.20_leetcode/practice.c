...
char** cellsInRange(char* s, int* returnSize) {
	int len = (s[3] - s[0] + 1) * (s[4] - s[1] + 1);
	char** ans = (char**)calloc(len, sizeof(char*));
	assert(ans);
	for (int i = 0; i < len; i++) {
		ans[i] = (char*)calloc(3, sizeof(char));
		assert(ans[i]);
	}
	*returnSize = 0;
	for (char col = s[0]; col <= s[3]; col += 1) {
		for (char row = s[1]; row <= s[4]; row += 1) {
			ans[*returnSize][0] = col;
			ans[*returnSize][1] = row;
			*returnSize += 1;
		}
	}
	return ans;
