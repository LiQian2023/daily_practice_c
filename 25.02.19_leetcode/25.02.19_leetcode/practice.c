...
void Partition(char* s, int left, int right, char* ans) {
	//字符串中的字符只不到一个时
	if (right - left <= 1) {
		return;
	}
	...
	if (single == -1) {
		int len = strlen(ans);
		if (right - left > len) {
			strncpy(ans, s + left, right - left);
		}
		return;
	}
	Partition(s, left, single, ans);
	Partition(s, single + 1, right, ans);
}
