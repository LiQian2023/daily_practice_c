...
int calc(int bit, int nor, int n, MapNode* map) {
	for (int i = 0; i < n; i++) {
		int x = map[i].x >> bit & 1;
		switch (map[i].op[0]) {
			case 'O':
...
int main() {
	...
	while (scanf("%d%d", &n, &m) == 2) {
		MapNode* map = (MapNode*)calloc(n, sizeof(MapNode));
		assert(map);
		// 获取门与值
		for (int i = 0; i < n; i++) {
			if (scanf("%s%d", map[i].op, &(map[i].x)) != 2) {
				printf("输入错误\n");
				break;
			}
		}
		...
		for (int i = 29; i >= 0; i--) {
			// 判断第i位是否可取
				// 确定第i位最终取值
			int bit0 = calc(i, 0, n, map);
			int bit1 = calc(i, 1, n, map);
			if (val + (1 << i) <= m && bit1 > bit0) {
				val += 1 << i;
				ans += bit1 << i;
			}
			else {
