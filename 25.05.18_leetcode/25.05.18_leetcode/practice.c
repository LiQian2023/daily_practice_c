...
void Get_Boundary(int* scores, int scoresSize, int* begin, int* end) {
	for (int i = 0; i < scoresSize; i++) {
		if (*begin > scores[i]) {
			*begin = scores[i];
		}
		else if (*end < scores[i]) {
			*end = scores[i];
		}
	}
}
int expectNumber(int* scores, int scoresSize) {
	int begin = scores[0], end = scores[0];
	Get_Boundary(scores, scoresSize, &begin, &end);
	int size = end - begin + 1;
	bool* hash = (bool*)calloc(size, sizeof(bool));
	assert(hash);
	int ans = 0;
	for (int i = 0; i < scoresSize; i++) {
		int key = scores[i] - begin;
		if (hash[key] == false) {
			ans += 1;
			hash[key] = true;
		}
