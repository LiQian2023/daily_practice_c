...
void DFS(int* materials, int** cookbooks, int** attributes, int n, int limit, int* cook, int* ans, int* delicious, int* full, int i) {
	for (int j = 0; j < 5; j++) {
	...
	if (i == n) {
		if (*full >= limit) {
			*ans = *ans > *delicious ? *ans : *delicious;
		}
		return;
	}
	...
	*delicious += attributes[i][0];
	*full += attributes[i][1];
	DFS(materials, cookbooks, attributes, n, limit, cook, ans, delicious, full, i + 1);
	// 不选
	for (int j = 0; j < 5; j++) {
		cook[j] -= cookbooks[i][j];
	}
...
int perfectMenu(int* materials, int materialsSize, int** cookbooks, int cookbooksSize, int* cookbooksColSize, int** attribute, int attributeSize, int* attributeColSize, int limit) {
	...
	int n = cookbooksSize;
	int cook[5] = { 0 };
	int delicious = 0, full = 0;
	DFS(materials, cookbooks, attribute, n, limit, cook, &ans, &delicious, &full, 0);
	return ans;
}